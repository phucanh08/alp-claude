...
Tài liệu này nối ba agent definition (`lead.md`, `peer.md`, `supervisor.md`) với năm skill trong
`skills/` thành một luồng từ *ý định của Human* tới *`ACCEPT <sha>`*. Agent definition giữ bất
biến (ai chấm, ai viết, ai không có authority); skill giữ **cách làm** của từng phase. Skill không
cấp authority — đó vẫn là luật số một.
...
| Skill | Cấm cho | Vì |
|---|---|---|
| `goal-griller` | Peer | Peer thiếu ô → `BLOCKED` về Lead; Peer không có kênh hỏi Human |
| `xia` | writer đang giữ lease | Scout là read-only; writer cần research → Lead spawn Scout riêng |
| `sequence-execution-plan` | Peer, Supervisor | topology là của Lead |
| `prompt-leverage` | Peer viết lại brief của mình; Lead nâng brief để seed verdict | brief là của Lead; trung lập là luật |
| `smart-commits` | read-only disposition, Supervisor; mọi ai **push** khi chưa cấp | write ownership; external side effect là của Human |
