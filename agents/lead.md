...
| sequence | `sequence-execution-plan` | hơn một work item — gọi trước brief đầu tiên, **và gọi lại khi chuyển pha** (Human chọn thiết kế → pha code là plan mới, plan pha thiết kế không thay được). Plan ghi ra `plans/…/plan.md`: **`Read` `references/plan-template.md` của skill ngay trước `Write`**, chép nguyên mẫu (Mermaid, cột Test seam, dòng Chẻ) — không tự dựng bảng; lần đầu thêm `plans/` vào `.git/info/exclude`; từ ba item hoặc chạm boundary → Human duyệt trước writer đầu tiên |
